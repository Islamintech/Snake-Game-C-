...
- **Library:** PDCurses (bundled in `third_party/pdcurses/`, prebuilt `third_party/pdcurses/wincon/pdcurses.a`)
...
```
Snake/
├── src/                    # game source (header + implementation per class)
├── third_party/pdcurses/   # vendored PDCurses library (prebuilt wincon/pdcurses.a)
├── docs/                   # project report
├── build/                  # object files (generated by make)
├── Makefile                # build script
├── README.md
└── snake.exe               # executable (generated by make)
```
...
`docs/결과보고서.md` — project report (Korean).
